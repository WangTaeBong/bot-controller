...
def is_authenticated(request: Request) -> bool:
    ...
    try:
        ...
        if not login_time:
            ...
            logging.warning("Session authentication failed: 'login_time' is missing in session data.")
            return False
        ...
        if time.time() - login_time > MAX_SESSION_DURATION:
            ...
            logging.warning("Session has expired based on the maximum allowed duration.")
            return False
        ...
        logging.warning("Session authentication failed: Invalid signature detected.")
        return False
    except Exception as e:
        # Log any other unexpected errors during the authentication check.
        logging.error(f"Unexpected error during session authentication: {e}")
        return False
...
async def verify_authentication(request: Request):
    ...
    if not is_authenticated(request):
        ...
        logging.warning("Unauthorized access attempt detected: redirecting to the login page.")
